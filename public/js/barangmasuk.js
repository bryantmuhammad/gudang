...
buttonTambahProduk.addEventListener("click", function (e) {
    ...
    const hargabeli = optionSelected.dataset.hargabeli;
    console.log(stok);
    ...
    if (!cekProduk) {
        ...
        html += `<td class="text-center">
               <input type="number" class="form-control" min="1" value="${hargabeli}" name="harga_beli[]">
           </td>`;
        html += ` <td class="text-center">
...
buttonSubmitForm.addEventListener("click", async function (e) {
    e.preventDefault();
    const form = new FormData(document.getElementById("formbarangmasuk"));

    let request = await fetch(`${CORE.baseUrl}/dashboard/barangmasuk/store`, {
        method: "post",
        headers: {
